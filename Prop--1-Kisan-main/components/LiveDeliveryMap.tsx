...
export const LiveDeliveryMap: React.FC<{
  pickup: { lat: number; lng: number };
  drop: { lat: number; lng: number };
  transporter?: { lat: number; lng: number } | null;
  className?: string;
}> = ({ pickup, drop, transporter, className }) => {
  const mapElRef = useRef<HTMLDivElement | null>(null);
  const mapRef = useRef<L.Map | null>(null);
  const pickupMarkerRef = useRef<L.Marker | null>(null);
  const dropMarkerRef = useRef<L.Marker | null>(null);
  const lineRef = useRef<L.Polyline | null>(null);
  const transporterMarkerRef = useRef<L.CircleMarker | null>(null);
  ...
  const bounds = useMemo(() => {
    ...
    if (transporter && Number.isFinite(transporter.lat) && Number.isFinite(transporter.lng)) {
      pts.push([transporter.lat, transporter.lng]);
    }
    const b = L.latLngBounds(pts as any);
    return b.isValid() ? b : null;
  }, [drop.lat, drop.lng, pickup.lat, pickup.lng, routePts, transporter?.lat, transporter?.lng]);
  ...
  useEffect(() => {
    ...
    lineRef.current = L.polyline(linePts as any, { color: '#16a34a', weight: 4, opacity: 0.85 }).addTo(map);
  ...
  useEffect(() => {
    ...
    if (transporter && Number.isFinite(transporter.lat) && Number.isFinite(transporter.lng)) {
      if (!transporterMarkerRef.current) {
        transporterMarkerRef.current = L.circleMarker([transporter.lat, transporter.lng], {
          radius: 9,
          weight: 3,
          color: '#f97316',
          fillColor: '#fb923c',
        ...
        transporterMarkerRef.current.setLatLng([transporter.lat, transporter.lng]);
      }
    } else {
      if (transporterMarkerRef.current) {
        transporterMarkerRef.current.remove();
        transporterMarkerRef.current = null;
      }
    }

    if (bounds) mapRef.current.fitBounds(bounds.pad(0.2));
  }, [bounds, drop.lat, drop.lng, pickup.lat, pickup.lng, routePts, transporter?.lat, transporter?.lng]);
